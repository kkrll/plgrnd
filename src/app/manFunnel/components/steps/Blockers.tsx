...
const blockersWeightLoss = [
  {
    ...
    pct: 47,
    color: "bg-gradient-to-r from-greenery-600 to-greenery-400 text-black",
  }, // The lime green
  {
  ...
  }, // The royal blue
  {
    ...
    pct: 20,
    color: "bg-gradient-to-r from-blue-700 to-blue-500 text-white w-[20%]",
  }, // The smaller blue
];
...
export default function BlockersStep() {
  ...
  const isMuscleGoal =
    funnelData.goal === "firmer-body" || funnelData.goal === "muscle";
        ...
        <div className="flex flex-col rounded-3xl bg-grey-800 overflow-hidden">
          <div className="flex flex-col gap-2 p-6">
            {blockers.map((item, i) => (
              <div
                key={i}
                className="relative h-10 flex items-center justify-end p-3 w-full bg-grey-900 rounded-xl overflow-hidden border border-gray-700/50"
              >
                {/* The Filled Bar */}
                <div
                  ...
                  style={{
                    width: `calc(${(item.pct / blockers[0].pct) * 100}% - 56px)`,
                  }}
                >
          ...
          <div className="flex flex-col gap-2 p-6 bg-gradient-to-r from-blue-600 to-blue-400">
            ...
            <p className="text-sm">
              {!isMuscleGoal &&
                "When workouts or healthy habits drop off after 1–2 weeks, results slow down even if motivation is high."}
            </p>
          </div>
        </div>
      </div>
