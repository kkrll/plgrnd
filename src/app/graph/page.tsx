...
const CustomizableYAxisChart = () => {
  ...
  const calculateDomain = (): [number | string, number | string] => {
    if (!useCustomDomain) {
      return includeZero ? [0, "auto"] : ["auto", "auto"];
    }
    ...
    const flatGraphCoeficent = calculateCoeficent();

    const calculatedMin = Math.max(
      0,
      bottomValue -
        ((actualDelta * belowMinPercentage) / 100) * flatGraphCoeficent
    );

    const calculatedMax = topValue + (actualDelta * aboveMaxPercentage) / 100;
              ...
              {useCustomDomain ? (
                <div>
                  ...
                  {!isPercentage ? (
                    ...
                    <>
                      <div className={!useCustomDomain ? "opacity-50" : ""}>
                        <div className="space-y-2">
                          <div>
